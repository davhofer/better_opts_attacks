...
def string_masks(
    ...
    prefix_span = (
        find_clean_token_span(tokenizer, full_text, adv_pre_init, final_tokens)
        if adv_pre_init
        else None
    )
    suffix_span = (
        find_clean_token_span(tokenizer, full_text, adv_suf_init, final_tokens)
        if adv_suf_init
        else None
    )
...
def initialize_adversarial_strings(
    ...
    if init_strategy_type == "random":
        ...
        if ascii_only:
            ...
            prefix_random_tokens = torch.tensor(
                [random.choice(ascii_toks) for _ in range(prefix_length)]
            )

            # Generate suffix tokens from ASCII-only set
            suffix_random_tokens = torch.tensor(
                [random.choice(ascii_toks) for _ in range(suffix_length)]
            )
        else:
